	...
	updateCart: function (id,qty){
		var params = {
			...
			success: function(cart) {
				...
				STB.Cart.updateLineCart(cart);
				$('.cartItemCount').html(cart.item_count);
			}
		};
		jQuery.ajax(params);
	},
	updateLineCart: function(cartLine){
		$('.money_line[data-id="'+ cartLine.id +'"]').html(cartLine.money_line);
        $('.cart__subtotal .money').html(cartLine.subtotal);
	},
